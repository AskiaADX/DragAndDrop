	...
	*/
	$.fn.adcDragndrop = function adcDragndrop(options) {
		...
		var $container = $(this),
			...
			initZindex = 100,
			numberOfDropZones = $container.find('.dropZone').length,
			removingItem = false,
		...
		function selectStatementSingle() {
			...
			if ( ($container.find('.nextStatement').css('display') === 'none' || $container.find('.nextStatement').length  === 0) || ( ($container.find('.nextStatement').css('display') != 'none' || $container.find('.nextStatement').length  > 0) && autoForward) ) nextIteration();
		}
		...
		// Use range if on
		if ( options.useRange ) {
			var maxNumber = $container.find('.responseItem').length  - options.numberNS;
			...
			$container.find('.responseItem').slice(0,(options.numberNS > 0)?0-options.numberNS:$container.find('.responseItem').length ).each(function( index ) {
		...
		$( ".dropZone" ).droppable({
			...
			drop: function( event, ui ) {
				...
				if  ( ( ($(".responseItem[data-value='" + containerID + "']").length > 0) || (checkrank) || ($('#' + iterations[$(ui.draggable).data('index')].id).val() >= 1) ) && areaExclusive ) {

					// Fail - revert to original position and reset value
					$(ui.draggable).data({'ontarget':false}).attr({'data-value':''}).css('margin','5px');
		...
		function noDrag(target) {
			...
			if ( !dragging ) {
				...
				if ( $(target).hasClass('responseActive') ) {
					...
					$('.dropZone, .startArea').off();

					$('.dropZone').on('mouseup', function (e) {
						setTarget(e, $(this).data('index'));
					});
					$('.startArea').on('mouseup', function (e) {
						setTarget(e, 'start');
					});
				}
		...
		function setTarget(e, destination ) {
			...
			if ( !removingItem ) {
				if ( destination === 'start' && target.hasClass('startArea') ) {
					...
					if ( $(clickActive).length > 0 ) $('#' + iterations[$(clickActive).data('index')].id).attr('value',target.attr('data-value')).val( target.attr('data-value') );

					var checkrank = checkRanking(parseInt(String(target.attr('data-value'))));
					// check if it already contains an item
					if ( !( ( ($(".responseItem[data-value='" + containerID + "']").length > 0) || (checkrank) || ($('#' + iterations[$(clickActive).data('index')].id).val() >= 1) ) && areaExclusive )) {
		...
		$('.dropZone').on('hover','.responseItemMini',
			function() {
			...
			function() {
				...
				if ( selectNextResponse && ($container.find('.responseActive').length < 1 || stackResponses ) ) {
		...
		$('.responseItem').on('mouseup', function() {
